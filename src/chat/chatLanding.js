...
export default class chatLanding extends React.Component {
            ...
            if (!emailArray.includes(followerDoc.data().email)) {
              ...
              if (searchArray.length) {
                searchArray.forEach(element => {
                  if (
                    element.email.trim() === followerDoc.data().email.trim()
                  ) {
  ...
  navigateToChat(selectedItem) {
    const uri =
      this.props.navigation.state.params.selectedItem &&
      this.props.navigation.state.params.selectedItem.item.url
        ? this.props.navigation.state.params.selectedItem.item.url
        : '';
          ...
          <ScrollView>
            <View style={{flex: 1}}>
              {this.state.user.isPrivateAccount == true ? (
                <TouchableOpacity
                  onPress={() => {
                    this.displayPendingMessageRequest();
                  }}>
                  ...
                  return (
                    <View style={styles.container}>
                      ...
                      <View style={styles.content}>
                        <Text
                          onPress={selectedItem => this.navigateToChat(item)}
                          style={styles.name}>
