...
export default class chatScreen extends React.Component {
  static navigationOptions = ({navigation}) => ({
    ...
    headerRight: (
      <TouchableOpacity
        onPress={() => navigation.state.params.handleImageClick()}>
        <FontAwesome5
          style={{
            color: 'white',
            fontSize: hp('3%'),
  ...
  componentDidMount() {
    ...
    Backend.loadMessages(message => {
      if (
        (message.userid === this.state.docRef &&
          message.user._id === Backend.getUid()) ||
        (message.userid === Backend.getUid() &&
          message.user._id === this.state.docRef)
      ) {
        this.setState({uri: ''});
  ...
  acceptRequest = () => {
    ...
    db.collection('signup')
      .doc(selectedItem.docRef)
      .collection('followers')
      .doc(item.email.trim())
      .set({email: item.email.trim()});
    db.collection('signup')
      .doc(item.docRef)
      .collection('following')
      .doc(selectedItem.email.trim())
      .set({email: selectedItem.email.trim()});
      ...
      docRef.get().then(function(doc) {
        if (doc.exists) {
          db.collection('signup')
            ...
            .collection('pendingFollowers')
            .doc(selectedItem.email.trim())
            .update({email: selectedItem.email.trim() + '_accepted'});
        }
      });
    }
  };
    ...
    docRef.get().then(function(doc) {
      if (doc.exists) {
        db.collection('signup')
          ...
          .collection('pendingFollowers')
          .doc(item.email.trim())
          .update({email: item.email.trim() + '_cancelled'});
      }
  ...
  renderChatFooter(props) {
    return (
      <>
        {this.state.selectedItem.isFollowing !== true &&
        this.state.messages.length &&
        this.state.userDetails.isPrivateAccount === true > 0 ? (
          <View
            style={{
              borderWidth: 1,
              borderColor: '#ff2700',
            ...
            <View style={styles.whisper}>
              <Text style={{color: '#ccc', fontWeight: 'bold'}}>
                {this.state.selectedItem.fullName} has something to whisper!
              </Text>
            </View>
            <View style={{flexDirection: 'row'}}>
              <View style={styles.accept}>
                <TouchableOpacity
                  style={{marginBottom: wp('25%'), flexDirection: 'row'}}
