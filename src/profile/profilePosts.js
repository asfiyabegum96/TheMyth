...
export default class ProfilePosts extends React.Component {
    ...
    componentDidMount() {
        ...
        this._unsubscribe = this.props.screenProps.navigation.addListener('willFocus', () => {
    ...
    fetchImages() {
        ...
        let params = context.props.screenProps;
        if (params.email) {
            photosRef.where('email', '==', params.email).where('isDeleted', '==', false).get().then(function (querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    let data;
                    const docNotEmpty = (doc.id, " => ", doc.data() != null);
                    if (docNotEmpty) data = (doc.id, " => ", doc.data());
